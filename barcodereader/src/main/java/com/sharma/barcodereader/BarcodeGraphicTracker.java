...
public class BarcodeGraphicTracker extends Tracker<Barcode> {
    ...
    public void onNewItem(int id, Barcode item) {
        mGraphic.setId(id);
        if (barcodeCaptureListener!=null)barcodeCaptureListener.onBarcodeDetected(item);
    }
    ...
    public void onUpdate(Detector.Detections<Barcode> detectionResults, Barcode item) {
        ...
        if (detectionResults != null && detectionResults.getDetectedItems().size() > 1) {
            ...
            if (barcodeCaptureListener != null) {
                SparseArray<Barcode> sparseArray = detectionResults.getDetectedItems();
                if (sparseArray == null) return;
                barcodeCaptureListener.onBarcodeDetected(sparseArray.valueAt(0));
            }
        }
    }
